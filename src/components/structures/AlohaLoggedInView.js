...
} from "matrix-react-sdk/lib/Keyboard";
...
const LoggedInView = React.createClass({
    ...
    switch (ev.keyCode) {
      ...
      case KeyCode.DOWN:
        if (ev.altKey && !ev.shiftKey && !ev.ctrlKey && !ev.metaKey) {
          const action =
            ev.keyCode == KeyCode.UP ? "view_prev_room" : "view_next_room";
  ...
  render: function() {
    ...
    let right_panel = "";
    console.log(
      "------------this.props.page_type ---------------" + this.props.page_type
    );
    ...
    if (
      this.state.syncErrorData &&
      this.state.syncErrorData.error.errcode === "M_RESOURCE_LIMIT_EXCEEDED"
    ) {
      topBar = (
        <ServerLimitBar
          kind="hard"
