...
import CallMediaHandler from 'matrix-react-sdk/lib/CallMediaHandler'
import sdk from 'matrix-react-sdk/lib/index'
import dis from 'matrix-react-sdk/lib/dispatcher'
import sessionStore from 'matrix-react-sdk/lib/stores/SessionStore'
import MatrixClientPeg from 'matrix-react-sdk/lib/MatrixClientPeg'
import SettingsStore from 'matrix-react-sdk/lib/settings/SettingsStore'
import RoomListStore from 'matrix-react-sdk/lib/stores/RoomListStore'
...
// @AHN
import AlohaClinicalTrialListView from './AlohaClinicalTrialListView'
import AlohaMyClinicalConsent from './AlohaMyClinicalConsent'
import AlohaMyClinicalProfile from './AlohaMyClinicalProfile'
...
const LoggedInView = React.createClass({
  displayName: 'AlohaLoggedInView',

  statics: {
    replaces: 'LoggedInView',
  },
  ...
  onSync: function(syncState, oldSyncState, data) {
    const oldErrCode =
      this.state.syncErrorData && this.state.syncErrorData.error && this.state.syncErrorData.error.errcode
    ...
    switch (ev.keyCode) {
      ...
      case KeyCode.DOWN:
        if (ev.altKey && !ev.shiftKey && !ev.ctrlKey && !ev.metaKey) {
          const action = ev.keyCode == KeyCode.UP ? 'view_prev_room' : 'view_next_room'
  ...
  _onDragEnd: function(result) {
    ...
    if (dest === 'tag-panel-droppable') {
      ...
      // state is shown.
      dis.dispatch(TagOrderActions.moveTag(this._matrixClient, draggableId, result.destination.index), true)
  ...
  render: function() {
    ...
    switch (this.props.page_type) {
      ...
      case PageTypes.CreateRoom:
        page_element = <CreateRoom onRoomCreated={this.props.onRoomCreated} collapsedRhs={this.props.collapseRhs} />
        if (!this.props.collapseRhs) right_panel = <RightPanel disabled={this.props.rightDisabled} />
        break

      case PageTypes.RoomDirectory:
        page_element = <RoomDirectory ref="roomDirectory" config={this.props.config.roomDirectory} />
        break
      ...
      case PageTypes.AlohaMyClinicalProfile:
        page_element = <AlohaMyClinicalProfile />
        break

      case PageTypes.AlohaMyConsent:
        page_element = <AlohaMyClinicalConsent />
        break
      ...
      case PageTypes.GroupView:
        ...
        if (!this.props.collapseRhs)
          right_panel = <RightPanel groupId={this.props.currentGroupId} disabled={this.props.rightDisabled} />
        break
    }
    ...
    if (this.state.syncErrorData && this.state.syncErrorData.error.errcode === 'M_RESOURCE_LIMIT_EXCEEDED') {
      ...
      !isGuest &&
      Notifier.supportsDesktopNotifications() &&
      !Notifier.isEnabled() &&
      !Notifier.isToolbarHidden()
    ) {
      ...
      >
        ...
        <DragDropContext onDragEnd={this._onDragEnd}>
          <div className={bodyClasses}>
            <LeftPanel collapsed={this.props.collapseLhs || false} disabled={this.props.leftDisabled} />
            <main className="mx_MatrixChat_middlePanel">{page_element}</main>
            {right_panel}
          </div>
        </DragDropContext>
      </div>
