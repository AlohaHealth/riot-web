...
module.exports = React.createClass({
  ...
  componentWillMount: function() {
    ...
    MatrixClientPeg.get().getThirdpartyProtocols().done(
      response => {
      ...
      err => {
        ...
        Modal.createTrackedDialog(
          "Failed to get protocol list from Home Server",
          "",
          ErrorDialog,
          {
            title: _t("Failed to get protocol list from Home Server"),
            description: _t(
              "The Home Server may be too old to support third party networks"
            )
          }
        );
      }
  ...
  getMoreRooms: function() {
    ...
    return MatrixClientPeg.get().publicRooms(opts).then(
      data => {
        if (
  ...
  removeFromDirectory: function(room) {
    ...
    if (alias) {
      desc = _t(
        "Delete the room alias %(alias)s and remove %(name)s from the directory?",
    ...
    if (!this.state.instanceId) {
      ...
      if (!fields) {
        ...
        Modal.createTrackedDialog("Unable to join network", "", ErrorDialog, {
          ...
          description: _t(
            "Riot does not know how to join a room on this network"
          )
        });
        return;
      }
      MatrixClientPeg.get().getThirdpartyLocation(protocolName, fields).done(
        resp => {
  ...
  getRows: function() {
    ...
    for (var i = 0; i < rooms.length; i++) {
      ...
      if (rooms[i].world_readable) {
        guestRead = (
          <div className="mx_RoomDirectory_perm">
            {_t("World readable")}
          </div>
        );
      }
  ...
  render: function() {
    ...
    if (
      ...
      const last_field = this.protocols[protocolName].location_fields.slice(
        -1
      )[0];
      instance_expected_field_type = this.protocols[protocolName].field_types[
        last_field
      ];
    }
